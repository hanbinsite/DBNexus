// DBNexus — Tab Management Module (TypeScript)
...
export function initTabs(): void {
    ...
    tabBar.addEventListener('click', (e: MouseEvent) => {
        ...
        if (tab && !(e.target as HTMLElement).closest('.tab-close')) {
...
export function createNewTab(): void {
    ...
    tabDiv.querySelector('.tab-close')!.addEventListener('click', (e: Event) => closeTab(tabId, e));
    ...
    const layoutAndFocus = () => {
        ...
        if (model) monacoEditor.setValue(model.getValue());
        monacoEditor.focus();
    };
...
export function activateTab(tabId: string): void {
    ...
    const tab = document.querySelector(`.tab[data-tab="${tabId}"]`) as HTMLElement | null;
...
export function closeTab(tabId: string, event?: Event): void {
    ...
    if (tab && tab.classList.contains('active')) {
        ...
        if (allTabs.length > 1) {
            ...
            if (prevTab) activateTab((prevTab as HTMLElement).dataset.tab || '');
        } else {
    ...
    if (activeWasClosed && state.activeTab) {
        setTimeout(() => {
            const activeTab = document.querySelector(`[data-tab="${state.activeTab}"]`) as HTMLElement | null;
            if (!activeTab) return;
