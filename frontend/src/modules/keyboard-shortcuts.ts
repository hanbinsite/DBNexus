// DBNexus — Keyboard Shortcuts Module (TypeScript)
...
let customShortcuts: Record<string, Shortcut> = {};

export function loadCustomShortcuts(): void {
    const saved = localStorage.getItem('dbnexus-shortcuts');
    if (saved) { try { customShortcuts = JSON.parse(saved); } catch { customShortcuts = {}; } }
}

export function saveCustomShortcuts(): void {
    localStorage.setItem('dbnexus-shortcuts', JSON.stringify(customShortcuts));
}

export function getShortcut(action: string): Shortcut | undefined {
    return customShortcuts[action] || DefaultShortcuts[action];
}

export function setShortcut(action: string, shortcut: Shortcut): void {
...
function matchShortcut(e: KeyboardEvent, action: string): boolean {
...
export function initKeyboardShortcuts(): void {
    ...
    document.addEventListener('keydown', (e: KeyboardEvent) => {
        ...
        const actions: Record<string, () => void> = {
            ...
            'close-tab': () => { if (state.activeTab) closeTab(state.activeTab, e); },
            ...
            'escape': () => {
                ...
                if (cm?.classList.contains('active')) closeConnectionDialog();
                if (sm?.classList.contains('active')) closeSettings();
                if (lm?.classList.contains('active')) closeLanguageDialog();
            },
        };
        for (const [action, handler] of Object.entries(actions)) {
            if (matchShortcut(e, action)) { e.preventDefault(); handler(); return; }
        }
    });
}

export function getShortcutList(): Array<Shortcut & { action: string; isCustom: boolean }> {
    const list: Array<Shortcut & { action: string; isCustom: boolean }> = [];
