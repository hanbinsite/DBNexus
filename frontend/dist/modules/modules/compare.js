...
exports.syncCompareResult = syncCompareResult;
async function compareTables(conn1, db1, table1, conn2, db2, table2) {
    showLoading('正在对比表数据...');
    try {
        ...
        showNotification('error', '对比失败: ' + (e.message || e));
    }
    finally {
        hideLoading();
    }
}
async function compareTableStructures(conn, db, table1, table2) {
    showLoading('正在对比表结构...');
    try {
...
async function compareQueryResults(conn1, db1, query1, conn2, db2, query2) {
...
function displayStructureDiff(result) {
    ...
    if (result.typeDiffs && result.typeDiffs.length > 0) {
        ...
        result.typeDiffs.forEach((diff) => { html += `<tr><td>${DomUtils.escapeHtml(diff.column)}</td><td>${DomUtils.escapeHtml(diff.type1)}</td><td>${DomUtils.escapeHtml(diff.type2)}</td></tr>`; });
...
async function syncCompareResult(conn2, db2, table2, differences) {
    if (!differences || differences.length === 0) {
        showNotification('info', '无差异需要同步');
        return;
    }
    if (!confirm(`确定要同步 ${differences.length} 条差异到目标表吗？`))
        return;
    showLoading('正在同步...');
    try {
        ...
        showNotification('error', '同步失败: ' + (e.message || e));
    }
    finally {
        hideLoading();
    }
