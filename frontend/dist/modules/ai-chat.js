 ...
 * Schema-aware AI conversation panel for database assistance
 */
    ...
    panel.innerHTML = `
        <div class="panel-header" style="display:flex;align-items:center;justify-content:space-between;padding:12px 16px;border-bottom:1px solid var(--border-primary);">
            <div style="display:flex;align-items:center;gap:8px;">
                <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="var(--accent-primary)" stroke-width="2">
            ...
            <button onclick="closeAIChatPanel()" style="background:none;border:none;cursor:pointer;color:var(--fg-muted);padding:4px;">
...
async function sendAIChatMessage() {
    ...
    try {
        ...
        const req = {
            sessionId: aiChatSession || 'default',
            message: message,
            config: state.activeConnection || {},
            database: state.selectedDatabase || '',
            context: {
                currentTable: state.currentTable?.name || '',
                currentQuery: getEditorValue ? getEditorValue() : '',
            },
        };
        ...
        if (resp.error) {
            addChatMessage('assistant', '错误: ' + resp.error);
        } else {
            addChatMessage('assistant', resp.message, resp.sql);

            // Show suggestions
            if (resp.suggestions && resp.suggestions.length > 0) {
                showChatSuggestions(resp.suggestions);
            }
        }
    } catch (error) {
        removeTypingIndicator(typingId);
        addChatMessage('assistant', '请求失败: ' + (error.message || error));
    }
}

function addChatMessage(role, content, sql) {
    const container = document.getElementById('aiChatMessages');
    if (!container) return;
    ...
    msg.style.cssText = isUser
        ? 'align-self:flex-end;background:var(--accent-primary);color:#fff;border-radius:var(--radius-md);padding:10px 14px;max-width:85%;font-size:13px;'
        : 'align-self:flex-start;background:var(--bg-secondary);color:var(--fg-primary);border-radius:var(--radius-md);padding:10px 14px;max-width:85%;font-size:13px;';
...
// Auto Index Recommendation UI
async function analyzeIndexes() {
    if (!state.currentTable || !state.activeConnection) {
        showNotification('warning', '请先选择表');
        return;
    }
    ...
    try {
        if (isWailsAvailable() && WailsAPI.recommendIndexes) {
            const result = await WailsAPI.recommendIndexes(
        ...
        showNotification('error', '分析失败: ' + (error.message || error));
    }
}

function displayIndexAnalysis(result) {
    const container = document.getElementById('aiChatMessages');
    if (!container) return;
    ...
    if (result.recommendations && result.recommendations.length > 0) {
        ...
        result.recommendations.forEach(rec => {
            const priorityColor = rec.priority === 'high' ? 'var(--accent-danger)' :
                                  rec.priority === 'medium' ? 'var(--accent-warning)' : 'var(--fg-muted)';
            html += '<div style="margin-bottom:6px;padding:8px;background:var(--bg-tertiary);border-radius:var(--radius-sm);border-left:3px solid ' + priorityColor + ';">';
            html += '<div style="font-weight:500;">' + rec.columnName + ' (' + rec.indexType + ')</div>';
            html += '<div style="font-size:11px;color:var(--fg-muted);">' + rec.reason + '</div>';
            html += '<div style="font-size:11px;color:var(--accent-success);">' + rec.estimatedImpact + '</div>';
            html += '<button onclick="insertSQLToEditor(\'' + rec.createSql.replace(/'/g, "\\'").replace(/\n/g, '\\n') + '\')" style="margin-top:4px;padding:3px 8px;font-size:10px;background:var(--accent-primary);color:#fff;border:none;border-radius:var(--radius-sm);cursor:pointer;">创建索引</button>';
            html += '</div>';
        });
        html += '</div>';
    }

    if (result.existingIndexes && result.existingIndexes.length > 0) {
        html += '<div style="margin-top:8px;font-size:11px;color:var(--fg-muted);">已有索引: ' + result.existingIndexes.join(', ') + '</div>';
    }
